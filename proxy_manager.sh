...
# Function to print fancy header - 120 chars width
print_header() {
    clear
    echo -e "${CYAN}╔══════════════════════════════════════════════════════════════════════════════════════════════════════════════════════╗${NC}"
...
# Function to print section header - 120 chars width
print_section_header() {
    local title=$1
    local title_length=${#title}
    local padding=$((117 - title_length))
...
# Function to print info box - 120 chars width
print_info_box() {
    local message=$1
    local color=${2:-$CYAN}
    local msg_length=${#message}
    local padding=$((117 - msg_length))
...
# Function to get network interfaces with IPs - 120 chars width
get_network_interfaces() {
    ...
    # Header with fixed width - 120 chars
    ...
    # Footer with fixed width - 120 chars
    echo -e "${CYAN}└────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘${NC}"
    echo
...
# Function to display system information with Dante status - 120 chars width
show_system_info() {
    ...
    # Function to print formatted line with exact width control - 120 chars
    print_info_line() {
        local label="$1"
        local value="$2"
        local color="$3"
        ...
        # Total box width is 119 characters (including borders)
        # Content area is 118 characters
        local padding=$((118 - content_len))
...
# Function to check service status - 120 chars width
check_service_status() {
    ...
    if systemctl is-active --quiet $DANTED_SERVICE 2>/dev/null; then
        if journalctl -u $DANTED_SERVICE --no-pager -n 5 --since "1 hour ago" 2>/dev/null | grep -q "."; then
            journalctl -u $DANTED_SERVICE --no-pager -n 5 --since "1 hour ago" 2>/dev/null | while read -r line; do
                # Truncate long log lines to fit in box - 120 chars
                if [[ ${#line} -gt 113 ]]; then
                    line="${line:0:110}..."
                fi
                local line_length=$((${#line} + 1))
                local line_padding=$((118 - line_length))
                printf "${CYAN}│${NC} ${GRAY}%s${NC}%*s${CYAN}│${NC}\n" "$line" $line_padding ""
            done
        else
            local no_logs="No recent logs found"
            local no_logs_length=$((${#no_logs} + 1))
            local no_logs_padding=$((118 - no_logs_length))
        ...
        local log_warning_padding=$((118 - log_warning_length))
    ...
    # Control options with box formatting - 120 chars
    ...
    for item in "${control_items[@]}"; do
        ...
        local item_padding=$((118 - item_length))
...
# Function to show users - 120 chars width
show_users() {
    print_header
    ...
    if [[ ${#users[@]} -eq 0 ]]; then
        ...
        for i in "${!users[@]}"; do
            ...
            local user_padding=$((118 - user_length))
...
# Function to delete users - 120 chars width
delete_users() {
    print_header
...
# Function to test proxies - 120 chars width
test_proxies() {
    print_header
    ...
    # Proxy test results with proper box formatting - 120 chars
    ...
    for i in "${!proxies[@]}"; do
        ...
        if [[ ${#display_proxy} -gt 50 ]]; then
            display_proxy="${display_proxy:0:47}..."
        fi
        ...
        # Calculate padding based on actual text length (không tính mã màu) - 120 chars
        ...
        local padding=$((110 - total_content_len))
        
        # Print the formatted line
        printf "${CYAN}│${NC} %s %-50s %b%*s${CYAN}│${NC}\n" \
...
# Function to uninstall Danted - 120 chars width
uninstall_danted() {
    print_header
...
# Main menu function - 120 chars width
show_main_menu() {
    print_header
    print_section_header "Main Menu"
    
    # Menu box with rounded corners - 120 chars
    echo -e "${YELLOW}┌─ Menu Options ────────────────────────────────────────────────────────────────────────────────────────────────────┐${NC}"
    
    # Menu items with proper padding - 120 chars
    local menu_items=(
